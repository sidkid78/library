...
- **Action**: Read and execute `cookbook/code_agent.md`
- **Examples**:
...
- **Action**: Read and execute `cookbook/research_agent.md`
- **Examples**:
...
- **Action**: Read and execute `cookbook/parallel_swarm.md`
- **Examples**:
...
If the user requests a general context handoff:

- **Condition**: Default when no specific type matches
- **Action**: Read and execute `cookbook/gemini_agent.md`
- **Examples**:
  - "Fork this conversation to a new agent"
  - "Hand off to another agent with context"
  - "Continue this work in a new agent"

## Tools

### Primary Tool: `agent_fork.py`

Location: `tools/agent_fork.py`

Execute with:

```bash
uv run tools/agent_fork.py --help
```

### Context Summarizer: `context_summarizer.py`

Location: `tools/context_summarizer.py`

Used automatically when forking with context handoff.

## Fork Summary Prompt

When forking with context, use the template at `prompts/fork_summary_user_prompt.md` to structure the handoff message.

**Important**: The summary prompt creates a YAML-structured context that includes:
